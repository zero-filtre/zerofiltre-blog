...
import { Observable, catchError, throwError, Subject, tap, map, of, shareReplay, forkJoin, finalize, combineLatest, takeUntil } from 'rxjs';
...
export class LessonComponent implements OnInit, OnDestroy {
  ...
  get canAccessCourse() {
    ...
    return (
      this.canBeEdited || this.courseService.canAccessCourse(user, this.course) || this.isSubscriber
    );
  }
  ...
  loadLessonData(lessonId: any) {
    ...
    this.lessonService.findLessonById(lessonId).subscribe({
      next: (lesson: Lesson) => {
        ...
        if(!this.canBeEdited) this.injectGiscus();
  ...
  manageEnrollment(user: User, lessonId: string) {
    if (!user || this.canBeEdited) return;
  ...
  ngOnInit(): void {
    ...
    this.courseInfosService.canBeEdited$.subscribe(canBeEdited =>
      {this.canBeEdited = canBeEdited});
    console.log("this.canBeEdited = " + this.canBeEdited);
