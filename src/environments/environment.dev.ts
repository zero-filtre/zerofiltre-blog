export const environment = {
  production: false,
  apiBaseUrl: 'https://blog-api-dev.zerofiltre.tech',
  blogUrl: 'https://blog-dev.zerofiltre.tech',
  GITHUB_CLIENT_ID: '9b6bffa9841d19dfd8aa',
  STACK_OVERFLOW_CLIENT_ID: '22742',
  gitHubRedirectURL: 'https://blog-dev.zerofiltre.tech/socials/auth/github',
  stackOverflowRedirectURL: 'https://blog-dev.zerofiltre.tech/socials/auth/stackOverflow',
  username: 'devUser',
  envName: 'development'
};